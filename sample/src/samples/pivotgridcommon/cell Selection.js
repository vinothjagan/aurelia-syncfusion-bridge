export class BasicUse {
  constructor() {
   ...
   if (args.detail.JSONRecords.length && (args.detail.JSONRecords[0].CSS.indexOf("colheader") != -1 || args.detail.JSONRecords[0].CSS.indexOf("rowheader") != -1 || args.detail.JSONRecords[0].CSS.indexOf("summary") != -1)) return false;
                else{
                ...
                for (var k = 0; k < cellvalue.length; k++) {
                    ...
                    var colHeaderCell = $(".e-pivotgrid").find("th[p*='" + cellvalue[0].Index.split(",")[0] + ",']").last();;
                    var isFound = false;
                    var isbrklp = false;
                    for (var col = parseInt(colHeaderCell.attr("p").split(",")[0]) ; col >= 0; col--) {
                        for (var r = parseInt(colHeaderCell.attr("p").split(",")[1] - 1) ; r >= 0; r--) {
                            if ($("#" + this._id).find("th[p*='" + col + "," + r + "']").length > 0) {
                                var element = $("#" + this._id).find("th[p*='" + col + "," + r + "']").find(".expand");                                
