export class Connectors {
    ...
    selectionChanged(event) {
        let diagram = this.diagram.widget;
        let ConnectorConstraints = ej.datavisualization.Diagram.ConnectorConstraints;
        let selectedConnectors = getSelectedConnectors(event.detail.selectedItems,diagram);
        let connectors = selectedConnectors.length ? selectedConnectors : diagram.model.connectors;
        updateCheckBox(this['dragable'], connectors, ConnectorConstraints.Drag,diagram);
        updateCheckBox(this['dragablesource'], connectors, ConnectorConstraints.DragSourceEnd,diagram);
        updateCheckBox(this['dragabletarget'], connectors, ConnectorConstraints.DragTargetEnd,diagram);
        updateCheckBox(this['dragablesegment'], connectors, ConnectorConstraints.DragSegmentThumb,diagram);
        updateCheckBox(this['bridge'], connectors, ConnectorConstraints.Bridging,diagram);

        let checked;
        let radius = connectors[0].cornerRadius;
        for (let i = 1; i < connectors.length; i++) {
            if (radius !== connectors[i].cornerRadius) {
                this.cornerRadius.widget.setModel({ enabled: false });
                checked = true;
                break;
            }
        }
        if (!checked) {
            this.cornerRadius.widget.setModel({ enabled: true });
            this.cornerRadius.widget.setModel({ value: radius });
        }
    }
    changeSegment(args) {
        let diagram =  this.diagram.widget;
        let selectedConnectors = getSelectedConnectors(diagram.model.selectedItems.children,diagram);
        let connectors = selectedConnectors.length ? selectedConnectors : diagram.model.connectors;
    ...
    changeBridgeDirection(args) {
        this.diagram.widget.setModel({ bridgeDirection: args.detail.value });
    }
    create(args) {
        let context = this;
        setTimeout(function () {
...
function getSelectedConnectors(selectedItems,diagram) {
    let connectors = [];
...
function updateCheckBox(checkbox, connectors, constraints,diagram) {
    let enabled;
    let connector;
    let checked;
    enabled = connectors[0].constraints & constraints ? true : false;
    if (connectors.length === 1) 
        checkbox.widget.setModel({ enabled: enabled });
    else {
        for (let i = 1; i < connectors.length; i++) {
            connector = connectors[i];
            if (diagram.getObjectType(connector) === 'connector') {
                if ((connector.constraints & constraints ? true : false) !== enabled) {
                    checkbox.widget.setModel({ enabled: false });
                    checked = true;
                    break;
                }
            }
    ...
    if (!checked) {
        checkbox.widget.setModel({enabled: true});     
        checkbox.widget.setModel({ checked: enabled });
    }
}
