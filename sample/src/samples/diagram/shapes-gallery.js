export class ShapesGallery {
    constructor() {
        this.objects =  basicShapes();
        this.snapSettings = { snapConstraints: ej.datavisualization.Diagram.SnapConstraints.None };
        this.pageSettings = {  scrollLimit: 'diagram'};
        this.defaultSettings = { nodes: { margin: { left: 10, right: 10, top: 10, bottom: 10 } } };
        this.target = 'shapeList';
        this.text = 'Basic Shapes';
    }
    select(event) {
        let shapes;
        if (event.detail.value === 'basicshapes') {
            shapes = basicShapes();
        } else if (event.detail.value === 'flowshapes') {
            shapes = flowShapes();
        } else if (event.detail.value === 'bpmnshapes') {
            shapes = bpmnShapes();
        }
        this.diagram.widget.setModel({
            nodes: shapes
        });
    }
}
function basicShapes() {
    let nodes = [];
    let BasicShapes = ej.datavisualization.Diagram.BasicShapes;
    let offsetx = 90;
    let offsety = 100;
    let count = 0;
    for (let i in BasicShapes) {
        count++;
        let basicnode = {
...
function bpmnShapes() {
    let nodes = [];
    let BPMNShapes = ej.datavisualization.Diagram.BPMNShapes;
    let offsetx = 100;
    let offsety = 100;
    let node;
    for (let k in BPMNShapes) {
